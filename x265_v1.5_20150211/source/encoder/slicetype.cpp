...
void Lookahead::vbvLookahead(Lowres **frames, int numFrames, int keyframe)
{
...
#if DEBUG_VBV_LOOKAHEAD
  {
...
#if DEBUG_VBV_LOOKAHEAD
    {
      ...
      {
        FILE* fp = fopen(GET_FILENAME(DEBUG_VBV_LOOKAHEAD), "a");
        fprintf(fp, "if:next_nonb poc=%lld idx=%d frames[%d]->i_planned_satd[%d]=%d ->i_planned_type[%d]=%d\n",
                frames[nextNonB]->frameNum,
                idx, nextNonB, idx, frames[nextNonB]->plannedSatd[idx],
                idx, frames[nextNonB]->plannedType[idx]);
        fclose(fp);
      }
      ...
      {
        FILE* fp = fopen(GET_FILENAME(DEBUG_VBV_LOOKAHEAD), "a");
        fprintf(fp, "for:i=%d poc=%lld idx=%d frames[%d]->i_planned_satd[%d]=%d ->i_planned_type[%d]=%d\n",
                i, frames[nextNonB]->frameNum,
                idx, nextNonB, idx, frames[nextNonB]->plannedSatd[idx],
                idx, frames[nextNonB]->plannedType[idx]);
        fclose(fp);
...
#define OUTPUT_CU_PIXEL(frm,idx,fnum) do{\
    FILE* pf = fopen(GET_FILENAME(DEBUG_FRAME_COST_OUTPUT), "a+");\
    fprintf(pf, "%s:CU(%lld=>%d:%d,%d:%d):\n",#frm,frm->frameNum,fnum, cux, cuy,idx);\
...
void EstimateRow::estimateCUCost(Lowres **frames, ReferencePlanes *wfref0, int cux, int cuy, int p0, int p1, int b, bool bDoSearch[2])
{
  ...
  OUTPUT_CU_PIXEL(fenc, 0,-1);
  OUTPUT_CU_PIXEL(fenc, 1,-1);
  OUTPUT_CU_PIXEL(fenc, 2,-1);
  OUTPUT_CU_PIXEL(fenc, 3,-1);
#endif
    if (p0 != p1)
    {
#if DEBUG_FRAME_COST_OUTPUT&&DEBUG_PIXEL_INFO&&DEBUG_CU_INFO&&KEEP_AS265_SAME_WITH_X265
    Lowres* fref0 = frames[p0];
    OUTPUT_CU_PIXEL(fref0, 0,fenc->frameNum);
    OUTPUT_CU_PIXEL(fref0, 1,fenc->frameNum);
    OUTPUT_CU_PIXEL(fref0, 2,fenc->frameNum);
    OUTPUT_CU_PIXEL(fref0, 3,fenc->frameNum);
    if(bBidir) {
      OUTPUT_CU_PIXEL(fref1, 0,fenc->frameNum);
      OUTPUT_CU_PIXEL(fref1, 1,fenc->frameNum);
      OUTPUT_CU_PIXEL(fref1, 2,fenc->frameNum);
      OUTPUT_CU_PIXEL(fref1, 3,fenc->frameNum);
